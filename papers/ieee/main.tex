...
\begin{abstract}
Spanish clinical natural language processing lacks open, well-documented question--answering resources focused on pregnancy and maternal health. This paper presents MaternaQA-es, a synthetic Spanish clinical question--answering dataset constructed from curated medical documents covering pregnancy, prenatal care, labor and delivery, postpartum care, fetal monitoring, obstetric emergencies, and related maternal-health topics. The dataset was created through a source-traceable process involving PDF text extraction, page-level cleaning, clinical-content filtering, chunk construction, synthetic question--answer generation, document-level data splitting, and post-hoc quality assessment. The current release contains 5,727 question--answer pairs derived from 1,953 source chunks and 57 source PDFs, with separate closed-book, grounded, and flat-audit variants to support supervised fine-tuning, evidence-conditioned question answering, dataset analysis, and retrieval-augmented generation research. We report corpus statistics, split composition, topic coverage, source-traceability controls, grounding analysis, and RAGAS-based estimates of faithfulness and answer relevancy. MaternaQA-es is intended as a research resource for Spanish maternal-health NLP and should not be used for direct clinical decision-making without independent expert validation.
\end{abstract}
...
Pregnancy and maternal health form a clinically important domain for language technologies. The domain includes preventive care, antenatal risk stratification, fetal monitoring, hypertensive disorders of pregnancy, diabetes in pregnancy, labor and delivery, postpartum complications, infection, medication safety, and newborn care. These topics require precise terminology and careful grounding because generated answers may be misinterpreted as clinical advice. As a result, dataset construction in this area must document not only the final examples, but also the provenance of source material, the rationale for inclusion and exclusion, the transformation from source text to QA pairs, quality controls, and responsible-use boundaries.

This paper introduces \textbf{MaternaQA-es}, a synthetic Spanish clinical QA dataset for pregnancy and maternal-health research. The contribution is not merely a software pipeline. The paper frames MaternaQA-es as a dataset creation and documentation effort: a curated resource with explicit source traceability, document-level splits, multiple publication variants, quality assessment, and stated limitations. This framing follows current calls for transparent dataset documentation, including datasheets for datasets \cite{Gebru2021Datasheets}, data statements for NLP \cite{Bender2018DataStatements}, and data cards \cite{Pushkarna2022DataCards}.
...
\begin{itemize}
    ...
    \item We report quality and documentation evidence, including leakage controls, topic coverage, grounding analysis, RAGAS-based faithfulness and answer-relevancy estimates, and explicit limitations for responsible use.
\end{itemize}

Although MaternaQA-es was designed to support downstream fine-tuning and benchmarking of open language models, this paper focuses on the dataset itself: its motivation, construction, documentation, quality assessment, and responsible release. Model adaptation experiments are treated as a separate downstream study.
...
\begin{table}[t]
\caption{Comparison with Related Biomedical QA Datasets}
\label{tab:related_datasets}
\centering
\footnotesize
\begin{tabular}{|p{0.18\columnwidth}|p{0.10\columnwidth}|p{0.18\columnwidth}|r|p{0.20\columnwidth}|c|}
\hline
\textbf{Dataset} & \textbf{Lang.} & \textbf{Domain} & \textbf{QA} & \textbf{QA type} & \textbf{Hum.} \\
\hline
SQuAD v1.1    & EN & General (Wiki)   & 98k+ & Extractive & Yes \\
Natural Qs.   & EN & General (Google) & 300k+ & Long-form  & Yes \\
PubMedQA      & EN & Biomed. lit.     & 211k  & Yes/no/maybe & Yes \\
MedQuAD       & EN & Consumer med.    & 47k   & QA pairs   & No$^*$ \\
MedMCQA       & EN & Medical exams    & 194k  & MC-QA      & No \\
BioASQ        & EN & Biomed. lit.     & 4k+   & Multiform  & Yes \\
RealMedQA     & EN & Clin. guidelines & 700   & Long-form  & Yes \\
\textbf{MaternaQA-es} & \textbf{ES} & \textbf{Maternal health} & \textbf{5,727} & \textbf{6 types} & \textbf{No} \\
\hline
\end{tabular}
...
\hline
Responsible release & The dataset includes limitations and non-intended-use statements. \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[t]
\caption{Source Corpus and Cleaning Summary}
...
\begin{table}[t]
\caption{Page Discard Reasons During Cleaning}
\label{tab:discard_reasons}
\centering
\begin{tabular}{|l|r|}
\hline
\textbf{Reason} & \textbf{Pages} \\
\hline
Needs OCR & 332 \\
Too short & 155 \\
Reference-heavy & 89 \\
...
The inclusion strategy favors documents with extractable Spanish clinical content and sufficient maternal-health relevance. Documents or pages with unreliable extraction are not silently repaired; instead, they are flagged or excluded so that the limitations of the current release remain visible.
...
The current final dataset contains 5,727 QA pairs. These examples are synthetic in phrasing, but not intended to be synthetic in factual basis. Each pair is generated from a source chunk and retains metadata for audit. A representative flat record includes fields such as \texttt{qa\_id}, \texttt{chunk\_id}, \texttt{source\_pdf}, \texttt{pages}, \texttt{topics}, \texttt{split}, \texttt{pregunta}, \texttt{respuesta}, \texttt{tipo}, \texttt{dificultad}, and \texttt{contexto\_fuente}.
...
\begin{table}[t]
\caption{Question Type Distribution Across Splits}
\label{tab:question_types}
\centering
\footnotesize
\begin{tabular}{|l|r|r|r|r|r|}
\hline
\textbf{Type} & \textbf{Train} & \textbf{Val.} & \textbf{Test} & \textbf{Total} & \textbf{\%} \\
\hline
...
\begin{table}[t]
\caption{Publication Variants of MaternaQA-es}
\label{tab:variants}
\centering
\scriptsize
\begin{tabular}{|p{0.25\columnwidth}|p{0.30\columnwidth}|p{0.28\columnwidth}|}
\hline
\textbf{Variant} & \textbf{Input View} & \textbf{Main Use} \\
\hline
...
\begin{table}[t]
\caption{RAGAS-Based Quality Estimates on Stratified Samples}
\label{tab:ragas}
\centering
\begin{tabular}{|l|r|r|r|}
\hline
\textbf{Split} & \textbf{Sample} & \textbf{Faith.} & \textbf{Rel.} \\
\hline
Train & 299 / 300 (5,093 total) & 0.7726 & 0.6466 \\
...
A pilot generator/evaluator comparison was conducted before the full-scale run. The pilot used a different evaluation pipeline (a custom structured-output judge running \texttt{gpt-5.4}) and reached faithfulness~0.9924 and relevancy~0.9947 on a small proof-of-concept sample drawn from early chunks. These scores are not directly comparable with the RAGAS estimates reported above: the pilot judge operated under a different rubric, was allowed to request answer revisions iteratively, and was not configured identically to the RAGAS faithfulness and answer relevancy definitions. We report the pilot result for transparency but treat the RAGAS-based post-hoc scores as the primary quality estimates for the released dataset. The generation run itself disabled intermediate evaluator scoring to remain within budget; therefore, final quality is assessed through the post-hoc sampling, grounding analysis, and explicit limitations described here.
...
\begin{table}[t]
\caption{Representative QA Examples from the Test Split}
\label{tab:examples}
\centering
\scriptsize
...
\begin{table}[t]
\caption{QA Dataset Split Composition}
\label{tab:qa_splits}
\centering
\begin{tabular}{|l|r|r|r|}
\hline
\textbf{Split} & \textbf{QA pairs} & \textbf{Chunks} & \textbf{PDFs} \\
\hline
...
The LM corpus split contains 1,997 train chunks, 115 validation chunks, and 111 test chunks, corresponding to actual split ratios of 0.8983, 0.0517, and 0.0499. Topic coverage is broad across maternal-health themes. Table~\ref{tab:topics} shows the distribution of the most frequent topic tags in the training split.

\begin{table}[t]
\caption{Top Topic Tags in the Training Split}
\label{tab:topics}
\centering
\begin{tabular}{|l|r|}
\hline
\textbf{Topic} & \textbf{QA pairs} \\
\hline
...
Although some gynecologic topics remain present because source documents include broader obstetrics and gynecology material, the paper frames the dataset as maternal-health-centered rather than pregnancy-only.
